...
- **`dives/<dive-name>/`** — Each dive gets its own folder with:
  ...
  - `dive_metadata.json` — `{ "id": "", "title": "...", "description": "...", "requiredResources": [{ "url": "md:_share/...", "alias": "..." }] }`. The `id` is left empty for new dives (populated on first deploy). Title is used to match existing dives for updates. `requiredResources` lists the shares the dive queries (must have at least one entry) — this is the source of truth for share dependencies and is passed to `MD_CREATE_DIVE` / `MD_UPDATE_DIVE_CONTENT` as `required_resources` on deploy.
...
- `REQUIRED_DATABASES` export **must be a single line** — the CI pipeline uses a regex to strip it before deploying. Multi-line will break deployment. The server-side share dependencies come from `requiredResources` in `dive_metadata.json`; the in-content `REQUIRED_DATABASES` constant is only used by local preview.
